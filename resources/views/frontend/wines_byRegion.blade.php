		...
		<div class="row">
			<div class="col-md-3">
				<div class="side-filter-bar">
					<h3 class="filter-link-title">{{$LANGUAGE==='EN'?'Regions':'产区'}}</h3>
					<ul class="filter-link-list">
					@foreach($region_links as $regionIdx=>$regionName)
						<li>
							<a href="/by-region/{{$regionName->RegionId}}">{{$regionName->Region}}</a>
						</li>
					@endforeach
					</ul>
			...
			<div class="col-md-9 col-sm-12">
				@if($showImpact!=='')
					@if(isset($regionInfo) && trim($regionInfo->RegionDesc)!=='' && trim($regionInfo->RegionMainImage)!=='' )
						@php
						$StlyMainImage = '';
						if($regionInfo->RegionMainImage){
		                    $findFile = str_replace(array("\\","/"),DIRECTORY_SEPARATOR,$regionInfo->RegionMainImage);
		                    if(file_exists(public_path($findFile))){
		                        $RegionMainImage = $findFile;
		                    }
		                }
						@endphp
						@if($RegionMainImage!=='')
				<div class="row category-intro-container">
					<div class="category-intro-desc col-md-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$LANGUAGE==='EN'&&trim($regionInfo->RegionEn)!=='' ? $regionInfo->RegionEn : $regionInfo->Region}}</h2>
                    	</div>
                    	<div class="category-description std d-none d-sm-block">
                			<p class="p1">
                				<span class="s1">{!! $LANGUAGE==='EN' && trim($regionInfo->RegionDescEn)!=='' ? $regionInfo->RegionDescEn : $regionInfo->RegionDesc !!}</span>
                			</p>
                		</div>
                	</div>
                	<div class="category-intro-img col-3 d-none d-sm-block">
                		<p class="category-image">
                			<img src="{!! asset($regionInfo->RegionMainImage) !!}" alt="{{$LANGUAGE==='EN'&&trim($regionInfo->RegionEn)!=='' ? $regionInfo->RegionEn : $regionInfo->Region}}" title="{{$LANGUAGE==='EN'&&trim($regionInfo->RegionEn)!=='' ? $regionInfo->RegionEn : $regionInfo->Region}}">
                		</p>
                	</div>
                </div>                	
                		@else
				...
				<div class="row category-intro-container">
					<div class="category-intro-desc col-sm-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$LANGUAGE==='EN'?'By region':'浏览不同产区的酒'}}</h2>
                        </div>
                    </div>
                </div>                
				@endif

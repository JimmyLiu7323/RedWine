		...
		<div class="row">
			<div class="col-md-3">
				<div class="side-filter-bar">
					@if($findVariety==='')
					<h3 class="filter-link-title">{{$LANGUAGE==='EN'?'varieties':'品种'}}</h3>
					<ul class="filter-link-list">
					@foreach($variety_links as $linkKey=>$child_links)
						...
						<li>
							...
							<a href="/wines/reds">{{$LANGUAGE==='EN'?'Reds':'红酒'}}</a>
								@endif
							<ul class="filter-children-link" @if($basic_catg)style='padding-left:0'@endif>
								@foreach($child_links as $varietyKey=>$variety)
								<li>
									<a href="/wines/reds?variety={{$variety['id']}}">{{$variety['name']}}</a>
								</li>
								@endforeach
							</ul>
							@elseif($linkKey==1)
							...
							<ul class="filter-children-link" @if($basic_catg)style='padding-left:0'@endif>
								@foreach($child_links as $varietyKey=>$variety)
								<li>
									<a href="/wines/whites?variety={{$variety['id']}}">{{$variety['name']}}</a>
								</li>
								@endforeach
							</ul>							
							@elseif($linkKey==2)
								@if(!$basic_catg)
							<a href="/wines/sparkling">{{$LANGUAGE==='EN'?'Sparkling':'气泡酒'}}</a>
								@endif
							<ul class="filter-children-link" @if($basic_catg)style='padding-left:0'@endif>
								@foreach($child_links as $varietyKey=>$variety)
								<li>
									<a href="/wines/spakrlings?variety={{$variety['id']}}">{{$variety['name']}}</a>
								</li>
								@endforeach
							</ul>							
							@endif
					...
					@if(!$basic_catg && count($style_links)>0)
						<li>{{$LANGUAGE==='EN'?'By Style':'不同风格'}}
							<ul class="filter-children-link">
						@foreach($style_links as $child_links)
								<li>
									<a href="/by-style/{{$child_links['id']}}">{{$child_links['name']}}</a>
								</li>
						@endforeach
							</ul>
						</li>
					...
					@if(!$basic_catg && count($style_links)>0)
						<li>{{$LANGUAGE==='EN'?'By Country':'出产国家'}}
							<ul class="filter-children-link">
						@foreach($country_links as $countryShort=>$country)
								<li>
									<a href="/by-country/{{$countryShort}}">{{$country}}</a>
								</li>
						@endforeach
							</ul>
						</li>
								...
								if(count($set_filter)>0){
									...
									if(isset($set_filter['style'])){
										...
										if(trim($tempFilterStyle)!=""){
											...
											if(strpos($set_filter['style'],$style->StyleId)===false)
												$link.=",".$style->StyleId;
										}
									}
								}
								...
								if(count($set_filter)>0){
									...
									if(isset($set_filter['region'])){
										...
										if(trim($tempFilterRegion)!=""){
											...
											if(strpos($set_filter['region'],$region->RegionId)===false)
			...
			<div class="col-md-9 col-sm-12">
				@if($showImpact!=='')
					@if($showImpact==='variety')
						@if(isset($varietyInfo) && $varietyInfo['Description']!=='' && $varietyInfo['MainImage']!=='' )
				<div class="row category-intro-container">
					<div class="category-intro-desc col-md-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$varietyInfo['Variety']}}</h2>
                    	</div>
                    	<div class="category-description std d-none d-sm-block">
                			<p class="p1">
                				<span class="s1">{!! $varietyInfo['Description'] !!}</span>
                			</p>
                		</div>
                	</div>
                	<div class="category-intro-img col-3 d-none d-sm-block">
                		<p class="category-image">
                			<img src="{!! asset($varietyInfo['MainImage']) !!}" alt="{{$varietyInfo['Variety']}}" title="{{$varietyInfo['Variety']}}">
                		</p>
                	</div>
                </div>
                		@endif
				...
				<div class="row category-intro-container">
					<div class="category-intro-desc col-md-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$basicCatgInfo['Catg']}}</h2>
                    	</div>
                    	<div class="category-description std d-none d-sm-block">
                			<p class="p1">
                				<span class="s1">{!! $basicCatgInfo['Description'] !!}</span>
                			</p>
                		</div>
                	</div>
                	<div class="category-intro-img col-3 d-none d-sm-block">
                		<p class="category-image">
                			<img src="{!! asset($basicCatgInfo['MainImage']) !!}" alt="{{$basicCatgInfo['Catg']}}" title="{{$basicCatgInfo['Catg']}}">
                		</p>
                	</div>
                </div>                	
                	@endif
				...
				<div class="row category-intro-container">
					<div class="category-intro-desc col-sm-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$LANGUAGE==='EN'?'BROWSE WINES':'浏览商品'}}</h2>
                        </div>
                    </div>
                </div>                
				@endif

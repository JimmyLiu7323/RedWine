			...
			<div class="col-md-9 col-sm-12">
				...
				<div class="row category-products">
					...
					@foreach($wines as $wineIdx=>$wine)
						<li class="item">
							@if($wine->ProductType==='wine')
                        	<a href="/wine/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Name}}" class="product-image">
                        	@elseif($wine->ProductType==='mix')
                        	<a href="/case/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Name}}" class="product-image">
                        	@endif
                        		<div class="crop">
                        		...
                        		<div class="f-fix">
                        			<div class="product-primary">
                        				<h2 class="product-name">
                        					@if($wine->ProductType==='wine')
                        					<a href="/wine/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Pname}}">{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Pname}}</a>
                        					@else
                        					<a href="/case/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Pname}}">{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Pname}}</a>
                        					@endif
                        				</h2>
                        				<div class="desc std">
                        					@if(trim($wine->PbriefDesc)!==''||trim($wine->PbriefDescEn)!=='')
                                				@if($LANGUAGE==='EN'&&trim($wine->PbriefDescEn)!=='')
                        					<div class="product-short-description">{{mb_substr($wine->PbriefDescEn,0,150)}} ... </div>
                        						@elseif(trim($wine->PbriefDesc)!=='')
                        					<div class="product-short-description">{{mb_substr($wine->PbriefDesc,0,150)}} ... </div>
                        						@endif
                        					<div class="mobile-readmore-link">
                        						@if($wine->ProductType==='wine')
                        						<a href="/wine/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Name}}" class="link-learn">Read More</a>
                        						@else
                        						<a href="/case/{{$wine->Pid}}" title="{{$LANGUAGE==='EN'&&trim($wine->NameEn)!==''?$wine->NameEn:$wine->Name}}" class="link-learn">Read More</a>
                        						@endif
                        					</div>
                        					@endif
                        				</div>
                        			...
                        			<div class="product-secondary">
                        				...
                        				<div class="action">
                        					<div class="qty-box">
                        						<label for="qty_{{$wine->Pid}}">{{$LANGUAGE==='EN'?'Qty':'数量'}}</label>
                        						<input type="number" name="qty" id="qty_wine_{{$wine->Pid}}" maxlength="12" value="1" min="0" step="1" title="Qty" class="input-text qty">
                        					</div>
                        					<button type="button" title="Add to Order" class="button btn-cart" onclick="quick_addItem('wine','{{$wine->Pid}}')">
                        						<span>

		...
		<div class="row">
			<div class="col-md-3">
				<div class="side-filter-bar">
					<h3 class="filter-link-title">{{$LANGUAGE==='EN'?'Styles':'风格'}}</h3>
					<ul class="filter-link-list">
					@foreach($style_links as $linkKey=>$style_link)
						<li>
							<a href="/by-style/{{$style_link->StyleId}}">{{$LANGUAGE==='EN' && trim($style_link->StyleEn)!=='' ? $style_link->StyleEn : $style_link->Style}}</a>
						</li>
					@endforeach
					</ul>
			...
			<div class="col-md-9 col-sm-12">
				@if($showImpact!=='')
					@if(isset($styleInfo) && trim($styleInfo->StyleDesc)!=='' && trim($styleInfo->StyleMainImage)!=='' )
						@php
						$StlyMainImage = '';
						if($styleInfo->StyleMainImage){
		                    $findFile = str_replace(array("\\","/"),DIRECTORY_SEPARATOR,$styleInfo->StyleMainImage);
		                    if(file_exists(public_path($findFile))){
		                        $StyleMainImage = $findFile;
		                    }
		                }
						@endphp
						@if($StyleMainImage!=='')
				<div class="row category-intro-container">
					<div class="category-intro-desc col-md-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$LANGUAGE==='EN'&&trim($styleInfo->StyleEn)!=='' ? $styleInfo->StyleEn : $styleInfo->Style}}</h2>
                    	</div>
                    	<div class="category-description std d-none d-sm-block">
                			<p class="p1">
                				<span class="s1">{!! $LANGUAGE==='EN' && trim($styleInfo->StyleDescEn)!=='' ? $styleInfo->StyleDescEn : $styleInfo->StyleDesc !!}</span>
                			</p>
                		</div>
                	</div>
                	<div class="category-intro-img col-3 d-none d-sm-block">
                		<p class="category-image">
                			<img src="{!! asset($styleInfo->StyleMainImage) !!}" alt="{{$LANGUAGE==='EN'&&trim($styleInfo->StyleEn)!=='' ? $styleInfo->StyleEn : $styleInfo->Style}}" title="{{$LANGUAGE==='EN'&&trim($styleInfo->StyleEn)!=='' ? $styleInfo->StyleEn : $styleInfo->Style}}">
                		</p>
                	</div>
                </div>                	
                		@else
				...
				<div class="row category-intro-container">
					<div class="category-intro-desc col-sm-9 col-12">
						<div class="page-title category-title">
                        	<h2>{{$LANGUAGE==='EN'?'By style':'浏览不同风格的酒'}}</h2>
                        </div>
                    </div>
                </div>                
				@endif
